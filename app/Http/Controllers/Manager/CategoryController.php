...
class CategoryController extends BaseAdminController{
    ...
    public function listView(Request $request){
        $this->menu();
        $this->title('Danh mục');
        $this->breadcrumb([['title' => 'Danh mục', 'link' => route('admin.category'), 'active' => 'active']]);
    ...
    public function getItem(Request $request, $id = 0){
        ...
        $this->breadcrumb([['title' => 'Danh mục', 'link' => \route('admin.category'), 'active' => ''], ['title' => $id == 0 ? 'thêm mới' : 'cập nhật', 'link' => \route('admin.category_edit', ['id' => $id]), 'active' => 'active']]);
        $data = array();
        if ($id > 0) {
