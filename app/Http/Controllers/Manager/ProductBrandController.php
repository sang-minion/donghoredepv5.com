...
class ProductBrandController extends BaseAdminController
{
    ...
    {
        parent::__construct();
        $this->parent = Category::getById(Category::getIdByKeyword(\CGlobal::key_nhan_hieu));
    }

    public function listView(Request $request)
    {
        $this->menu();
        $this->title('Nhãn hiệu sản phẩm');
        $this->breadcrumb([['title' => 'Nhãn hiệu sản phẩm', 'link' => route('admin.product_brand'), 'active' => 'active']]);
        ...
        return view('Manager.product_brand.list', ['search' => $search, 'data' => $dataSearch, 'total' => $total, 'paging' => $paging, 'optionStatus' => $optionStatus]);
    }

    public function getItem(Request $request, $id = 0)
    {
        $this->menu();
        $this->title($id == 0 ? 'Thêm nhãn hiệu mới' : 'Cập nhật nhãn hiệu');
        $this->breadcrumb([['title' => 'Cập nhật nhãn hiệu', 'link' => \route('admin.product_brand'), 'active' => ''], ['title' => $id == 0 ? 'thêm mới' : 'cập nhật', 'link' => \route('admin.product_category_edit', ['id' => $id]), 'active' => 'active']]);
        $data = array();
        if ($id > 0) {
        ...
        return view('Manager.product_brand.add', ['id' => $id, 'data' => $data, 'optionStatus' => $optionStatus]);
    }
    ...
    public function postItem(Request $request, $id = 0)
    {
        ...
        return redirect()->route('admin.product_brand');
    }
