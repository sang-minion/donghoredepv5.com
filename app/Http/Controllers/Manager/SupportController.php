...
class SupportController extends BaseAdminController
{
    ...
    public function listView(Request $request){
        $this->menu();
        $this->title('Hỗ trợ khách hàng');
        $this->breadcrumb([['title' => 'Hỗ trợ khách hàng', 'link' => route('admin.support'), 'active' => 'active']]);
        ...
        $search['static_parent_key']=\CGlobal::key_ho_tro_khach_hang;
        ...
        return view('Manager.support.list', ['search' => $search, 'data' => $dataSearch, 'total' => $total, 'paging' => $paging, 'optionStatus' => $optionStatus]);
    }
    public function getItem(Request $request, $id = 0){
        $this->menu();
        $this->title($id == 0 ? 'Thêm mới' : 'Cập nhật');
        $this->breadcrumb([['title' => 'Hỗ trợ khách hàng', 'link' => \route('admin.support'), 'active' => ''], ['title' => $id == 0 ? 'thêm mới' : 'cập nhật', 'link' => \route('admin.support_edit', ['id' => $id]), 'active' => 'active']]);
        ...
        return view('Manager.support.add', ['id' => $id, 'data' => $data, 'optionStatus' => $optionStatus]);
    }
    ...
    public function postItem(Request $request, $id = 0){
        ...
        $data = array('static_title' => addslashes($request->static_title),
            ...
            'static_parent_key'=>\CGlobal::key_ho_tro_khach_hang,
