...
@section('content')
    <form action="" class="form-horizontal frmAdd" method="POST" id="frmAdd" name="frmAdd" enctype="multipart/form-data">
        {{ csrf_field() }}
        <div class="form-group{{isset($errors)&& $errors->has('category_parent_id') ? ' has-error' : '' }}">
            ...
            <div class="col-md-6 col-sm-9">
                ...
                @if (isset($errors)&&$errors->has('category_parent_id'))
                    <span class="help-block">
                                        <strong>{{ $errors->first('category_parent_id') }}</strong>
                                    </span>
                @endif
            </div>
        </div>
