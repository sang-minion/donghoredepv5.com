...
@section('content')
        ...
        <form name="frmSearch" class="frmSearch" id="frmSearch" method="GET">
            <div class="row">
                ...
                <div class="col-lg-2 col-md-2 col-sm-3">
                    ...
                    <div>
                        <select name="category_parent_id" class="form-control input-sm">
                            {!! $optionParent !!}
                        </select>
                    </div>
                </div>
    ...
    <div class="row menu-option">
        <div class="col-lg-6 col-md-6 col-sm-6"><h5>Quản lý: Danh mục [tổng số: {{$total}}]</h5></div>
        <div class="col-lg-6 col-md-6 col-sm-6" style="text-align: right">
            <a href="{{route('admin.category_edit')}}"> <i class="fa fa-plus" title="thêm mới"></i> </a>
            <a href="javascript:void(0)"> <i class="fa fa-trash" id="deleteMoreItem" title="xóa item"></i> </a>
        </div>
    </div>
    <div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12">
            <form action="{{route('admin.category_delete') }}" id="formListItem" name="formListItem" method="POST">
                {{ csrf_field() }}
                        ...
                        @foreach($data as $k=>$item)
                            <tr>
                                ...
                                <td><input class="checkItem" name="checkItem[]" type="checkbox"
										   ...
										   <td>@if($item->category_media!='')<img src="{{ThumbImg::thumbBaseNormal(Category::FOLDER,$item->category_id,$item->category_media,800,600,'',true,true,true)}}"  rel="{{ThumbImg::thumbBaseNormal(Category::FOLDER,$item->category_id,$item->category_media,800,600,'',true,true,true)}}" id="showIMG" style="width:100%; min-width:30px"/>@endif</td>		  
                                <td>@if($item->category_media!='') <a href="{{Funclip::buildLinkCategory($item->category_id,$item->category_keyword)}}" target="_blank" title="{{$item->category_title}}" > {{$item->category_title}} </a>@else {{$item->category_title}} @endif</td>
                                <td>
                                    @foreach($arrParent as $k=>$v)
                                        @if($item->category_parent_id==$k)
                                            {{$v}}
                                        @endif
                                    @endforeach
                                </td>
