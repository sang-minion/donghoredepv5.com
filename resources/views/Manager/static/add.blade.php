...
@section('content')
    <form action="" class="form-horizontal frmAdd" method="POST" id="frmAdd" name="frmAdd" enctype="multipart/form-data">
        {{ csrf_field() }}
        <div class="form-group{{isset($errors)&& $errors->has('static_parent_key') ? ' has-error' : '' }}">
            <label for="static_parent_key" class="col-md-2 col-sm-3 control-label">Chọn dah mục</label>
            <div class="col-md-6 col-sm-9">
                <select class="form-control" name="static_parent_key">
                    {!! $optionKey !!}
                </select>
                @if (isset($errors)&&$errors->has('static_parent_key'))
                    <span class="help-block">
                                        <strong>Bạn chưa chọn danh mục</strong>
                                    </span>
                @endif
            </div>
        </div>
